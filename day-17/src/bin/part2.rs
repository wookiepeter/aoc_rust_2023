...
fn process(input: &str) -> String {
    ...
    while let Some(state) = data.queue.pop() {
        if state.last_step_horizontal {
            add_steps_in_direction(&state, &mut data, Direction::Down, 3..10);
            add_steps_in_direction(&state, &mut data, Direction::Up, 3..10);
        } else {
            add_steps_in_direction(&state, &mut data, Direction::Right, 3..10);
            add_steps_in_direction(&state, &mut data, Direction::Left, 3..10);
        }
    }
